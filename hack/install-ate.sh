...
render_atenet_egress_manifest() {
  ...
  # patch reads the manifest file and the injection patch reads stdin. The
  # cordon wrapper comes last, over the spliced stream, so the node pinning
  # reaches every variant.
  if additional_egress_extproc_enabled && egress_credential_injection_enabled; then
    patch_atenet_egress_manifest | patch_atenet_egress_inject | render_manifests - | run_ko resolve -f -
    ...
    patch_atenet_egress_inject < "$(atenet_egress_manifest)" | render_manifests - | run_ko resolve -f -
  else
