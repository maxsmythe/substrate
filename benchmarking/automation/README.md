...
1. CronJob fires on the orchestration cluster; pod starts (orchestrator +
   DIND sidecar).
2. Orchestrator waits for DIND, sources the baked-in `ate-dev-env.sh`, runs
...
The wizard collects test-cluster values, writes `scratch/ate-dev-env.sh`,
builds + pushes the orchestrator image to
`${KO_DOCKER_REPO}/substrate-benchmark-orchestrator:<short-commit>` (with a
`-dirty` suffix if `benchmarking/automation/` has uncommitted changes), then
renders `scratch/cronjob.yaml` with that pinned image reference.
...
**Runner Job pod** (KSA `benchmark-runner` in namespace `monitoring` on the
test cluster's project) needs `roles/storage.objectCreator` on the destination
bucket so `runner.py` can upload results:

```bash
RUNNER_PRINCIPAL="principal://iam.googleapis.com/projects/<TEST_PROJECT_NUMBER>/locations/global/workloadIdentityPools/<TEST_PROJECT_ID>.svc.id.goog/subject/ns/monitoring/sa/benchmark-runner"
...
the test list changes. Edit `tests.yaml`, then apply just the ConfigMap:

```bash
kubectl --context=<orchestration-cluster> -n substrate-benchmark \
  create configmap substrate-benchmark-tests \
  --from-file=tests.yaml=benchmarking/automation/tests.yaml \
  --dry-run=client -o yaml | kubectl apply -f -
```

Re-running `setup.sh` also regenerates `scratch/cronjob.yaml` (which contains
both the CronJob and the current ConfigMap content), so applying that file
picks up tests.yaml edits too.
